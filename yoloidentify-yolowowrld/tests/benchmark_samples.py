...
def _iter_sample_images(samples_dir: Path) -> list[tuple[Path, str]]:
    ...
    skip_tags = {
        item.strip()
        for item in os.environ.get("SKIP_TAGS", "").split(",")
        if item.strip()
    }
...
def run_benchmark(samples_dir: Path) -> list[OneResult]:
    ...
    with TestClient(app) as client:
        for image_path, tag in images:
            ...
            coarse = data.get("coarse_result") or {}
            labels = coarse.get("labels") or []
            pred = coarse.get("category_name") or (labels[0] if labels else "")
            ok = tag in labels or pred == tag
