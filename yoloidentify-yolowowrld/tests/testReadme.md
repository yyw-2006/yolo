...
说明：`benchmark_samples.py` 会把 `samples/` 下的每个子目录名当作标签，预测结果的 `coarse_result.labels` 包含该标签则记为正确。
...
```bash
cd "/home/yao/桌面/yoloidentify/yoloidentify"
SAMPLES_DIR="/abs/path/to/samples" python3 -m tests.benchmark_samples
```
...
## 4) 常见问题
- 找不到样本：确认 `yoloidentify/samples/` 下存在子目录（如 `normal/`、`terrorism`、`nazi_symbol`）且里面有图片文件。
